...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function updateChatButtonState() {
        const checkedCount = document.querySelectorAll(".contact-item .checkbox.checked").length;
        if (checkedCount > 0) {
    ...
    function removeChip(contactName) {
        const chip = document.querySelector(`.selected-chip[data-name="${contactName}"]`);
        if (chip) {
            chip.remove();
        }
    }
    function deselectContact(contactName) {
        ...
        document.querySelectorAll(".contact-item").forEach((item) => {
            var _a, _b;
            if (((_a = item.querySelector(".info .name")) === null || _a === void 0 ? void 0 : _a.textContent) === contactName) {
                (_b = item
                    .querySelector(".checkbox")) === null || _b === void 0 ? void 0 : _b.classList.remove("checked");
            }
        });
    ...
    search.addEventListener("input", (e) => {
        ...
        filteredContacts = suggestedContacts.filter((s) => s.name.toLowerCase().includes(inputValue));
...
document.addEventListener("click", (e) => {
    const target = e.target;
    const isClickInsideMeta = metaContainer.contains(target) || meta.contains(target);
